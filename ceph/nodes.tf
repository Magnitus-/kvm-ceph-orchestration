...
resource "libvirt_volume" "ceph_4" {
  name             = "ceph-4"
  pool             = "ceph"
  size             = 10 * 1024 * 1024 * 1024
  base_volume_pool = "ceph"
...
resource "libvirt_volume" "ceph_4_data_1" {
  name             = "ceph-4-data-1"
  pool             = "ceph"
  size             = 10 * 1024 * 1024 * 1024
  format           = "qcow2"
...
resource "libvirt_volume" "ceph_4_data_2" {
  name             = "ceph-4-data-2"
  pool             = "ceph"
  size             = 10 * 1024 * 1024 * 1024
  format           = "qcow2"
...
resource "libvirt_volume" "ceph_4_data_3" {
  name             = "ceph-4-data-3"
  pool             = "ceph"
  size             = 10 * 1024 * 1024 * 1024
  format           = "qcow2"
...
resource "libvirt_volume" "ceph_4_data_4" {
  name             = "ceph-4-data-4"
  pool             = "ceph"
  size             = 10 * 1024 * 1024 * 1024
  format           = "qcow2"
...
locals {
  ceph_cluster = [
    ...
    {
      ip = local.params.network.machines.ceph_nodes.3.ip
      domain = "server-4.ceph.lan"
      monitor = true
      manager = true
      admin   = true
      osd     = true
...
module "ceph_4" {
  ...
  name = "ceph-4"
  hostname = {
    hostname = "server-4.ceph.lan"
    is_fqdn  = true
  }
  vcpus = 2
  memory = 12288
  volume_ids = [
    libvirt_volume.ceph_4.id,
    libvirt_volume.ceph_4_data_1.id,
    libvirt_volume.ceph_4_data_2.id,
    libvirt_volume.ceph_4_data_3.id,
    libvirt_volume.ceph_4_data_4.id
  ]
  libvirt_networks = [{
    network_name = "ceph"
    network_id = ""
    prefix_length = split("/", local.params.network.addresses).1
    ip = local.params.network.machines.ceph_nodes.3.ip
    mac = local.params.network.machines.ceph_nodes.3.mac
    gateway = local.params.network.gateway
    dns_servers = [
      local.params.network.machines.ops_nodes.0.ip,
      local.params.network.machines.ops_nodes.1.ip,
